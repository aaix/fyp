...
async def lazy_init():
    ...
    await dataservices_bigpicture.wait_for()
...
class _DynamicLazyGRPC[T: _GRPCStub]:
    ...
    def __call__(self, bucket: id_t | None = None) -> T:
        return self._stub(bucket)
    ...
    def _stub(self, bucket: id_t | None) -> T:
        key = self._bucket_to_remote(bucket)
        return self._stubs[key]


    def _bucket_to_remote(self, bucket: id_t | None) -> str:
        # use a random key if we dont need persistent routing
        key = id_uuid(bucket) or uuid.uuid4() if bucket else uuid.uuid4()
        return self.BIG_PICTURE.get_node(key)
